<?php 
	...
	if(isset($_POST['customize_fields_motors']))
	{	
		if(!empty($_SESSION['myuser']['us_id'])) {
			$user_id = $_SESSION['myuser']['us_id'];
			$sqlQry =  $connect->query("select * from user where us_id=".$user_id);
			if($sqlQry->num_rows) {
				$d_row = $sqlQry->fetch_assoc();
			$sel="SHOW COLUMNS from motercycle";
			$result = $connect->query($sel);
			if($result->num_rows>0) {
				
					$checked=json_decode($d_row['show_customise_motor_fields'],true);
				...
				while($row = $result->fetch_assoc()){
					...
					?>
					<div class="form-group">
						<input type="checkbox" class="checkboxField" id="<?php echo $row['Field']; ?>1" name="Fnames[<?php echo $row['Field']; ?>]" value="1" <?php if(in_array($row['Field'],$checked)) echo "checked";?>>
						<label for="<?php echo $row['Field']; ?>1" class="Lnames">
							<b class="lowercase">
								<?php 
								if (strpos($row['Field'], 'AUM_') !== false) {
										echo str_replace("AUM_", "",$row['Field']);
									}else{
										echo $row['Field'];
									}
								?>
	...
	if(isset($_POST['SubmitSelectMotorFields']))
	{
		// print_r($_POST);  die;
		if(!empty($_SESSION['myuser']['us_id'])) {
			$user_id = $_SESSION['myuser']['us_id'];
			if(!empty($_POST['Fnames'])) {
				...
				if(!empty($selectArr)) {
					$JEselectArr = json_encode($selectArr);
					$connect->query("update user set show_customise_motor_fields='".$JEselectArr."' where us_id=".$user_id);
					echo "1";
				}
			} else{
				echo "Please select one field to show in search result.";
	...
	if(isset($_POST["actioncheck"])){
		$user_id = $_SESSION['myuser']['us_id'];
		
		$q = $connect->query("UPDATE `user` SET `is_kType_show`=".$_POST['checked']." WHERE us_id=".$user_id);
		if($q) {
			echo $_POST['checked'];
		}
	}
	...
	if(isset($_POST["search_data"]))
	{
		...
		if($Submodel!=''){ 
			...
			$str=$str." Submodel Like '%".$Submodel."%' ";
		}
		if($Variant!=''){ 
			if($str!=''){ 
				$str=$str.' AND '; 
			}
			$str=$str." Variant Like '%".$Variant."%' ";
		}
		if($year_from!=''){ 
			if($str!=''){ 
				$str=$str.' AND '; 
			}
			$str=$str." Year>=".$year_from;
		}
		if($year_to!=''){ 
			if($str!=''){ 
				$str=$str.' AND '; 
			}
			$str=$str." Year<=".$year_to;
		}
		
		$asql = "SELECT * FROM data  WHERE ".$str.";";
		$aresult = $connect->query($asql);
		
		/*-----*/ $d_row=array(); $KType_display='none'; $ktype_checked='';
		if(!empty($_SESSION['myuser']['us_id'])) {
			$user_id = $_SESSION['myuser']['us_id'];
			$sqlQry =  $connect->query("select * from user where us_id=".$user_id);
			if($sqlQry->num_rows) {
			...
			if(!empty($d_row)) {
				$fields=json_decode($d_row['show_customise_fields'],true);
				//print_r($fields);
			
				if($d_row['is_kType_show']==1) {
					$KType_display='block';
					$ktype_checked='checked';
				}
			}
		}
		/*-----*/
		...
		if ($aresult->num_rows > 0) {
			// output data of each row
			$data['html'].= '<div class="row sg_cpyR" id="EpidWrap"><div class="col-md-12"><label class="col-md-2">ePID</label>
			...
			$data['html'].= '<div class="row sg_cpyR" id="KTypeAccess"><div class="col-md-12"><div class="col-md-1"><input type="checkbox" '.$ktype_checked.' id="ktype-checked"></div> <label for="ktype-checked" class="col-md-8 KtypeLabel">I confirm I have legal authority to access and use K-Type codes</label></div></div>';
			
			$data['html'].= '<div class="row sg_cpyR" id="K_Type" style="display:'.$KType_display.';"><div class="col-md-12"><label class="col-md-2">K-Type</label>
			<div class="col-md-8">
			<input type="text" class="form-control get_all_ktype" /></div>
			...
			<thead>
			  <tr><th class="nosort space"><input type="checkbox" id="selectall" title="Select all"></th>';
			  if(!empty($fields)){
			...
			while($row = $aresult->fetch_assoc())
			{
				...
				$data['html'].= "<tr>
				<td><input type='checkbox' name='select[]' class='sgt_chk' epid='$mepid' ktype='$mktype' > </td>";
	...
	if(isset($_POST["NeedHelpMail"])) {
		...
		if(!empty($_POST['contact'])){
			$to = "info@zellis.com.au";
			$subject = "Locked ZELLIS Auto account";
		}
		else{
			$to = 'tim@zellis.com.au';
