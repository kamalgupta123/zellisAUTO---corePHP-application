<?php 
		...
		if(!empty($_SESSION['admin']['id'])) {
			...
			if(!empty($_POST['Fnames'])) {
				...
				if(!empty($selectArr)) {
					...
					$connect->query("update admin set show_customise_fields='".$JEselectArr."' where id=".$user_id);
					echo "1";
				}
			} else{
	...
	if(isset($_POST['customize_fields']))
	{
		if(!empty($_SESSION['admin']['id'])) {
			$user_id = $_SESSION['admin']['id'];
			$sqlQry =  $connect->query("select * from admin where id=".$user_id);
			if($sqlQry->num_rows) {
	...
	if(isset($_POST['UploadCSVData'])) {
		...
		if(empty($data["errors"])){
			...
			$Usql = $connect->query("select * from zl_user_csv_info where user_id=".$user_id." and user_type=1");
			if($Usql->num_rows>0) {
				// if user already uploads csv then delete the table
				$connect->query("delete from zl_user_csv_info where user_id=".$user_id." and user_type=1");
				$connect->query("drop table zl_ad_csv_".$user_id);
			}
				...
				while(($data_f = fgetcsv($file,  0, ",","'")) !== FALSE)
				{
					if($i==0) {
						...
						if(!empty($arr)) {
							...
							$sql = "insert into zl_user_csv_info (user_id,user_type,csv_name,fields_name,created_on) VALUES ('".$user_id."',1,'".$new_name1."','".$json_str."','".$cDate."')";
							// echo $sql;
							$aresult = $connect->query($sql);
							// create table as csv file uploaded
							$Csql = "CREATE TABLE zl_ad_csv_".$user_id." (";
							...
							$valSql = "insert into zl_ad_csv_".$user_id." (`".implode("`, `", array_keys($arr))."`) VALUES";
						} 
					} else {
	...
	if(isset($_POST['SearchCSVSubmit'])) {
		...
		if(!empty($str)) {
			...
			$asql = "SELECT * FROM zl_ad_csv_".$user_id."  WHERE ".$str.";";
